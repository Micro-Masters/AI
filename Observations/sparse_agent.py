...
class SparseAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        #    if obs.observation['minimap'] != self.previous_observations:
        print("feature_unit_observations: " + str(obs.observation['feature_units']))
        #self.previous_observations = obs.observation['single_select'][0][1]
        ...
        #print("feature_screen " + str(obs.observation['feature_screen'][0][5]))
        ...
        if self.move_number == 0:
            ...
            #print("feature_minimap " + str(obs.observation['feature_minimap'][0][5]))
            for i in range(0, len(enemy_y)):
                y = int(math.ceil((enemy_y[i] + 1) / 32))
                x = int(math.ceil((enemy_x[i] + 1) / 32))
                #print("enemy_y " + str(enemy_y) + " enemy_x " + str(enemy_x))
                self.enemyx = enemy_x[i]
                self.enemyy = enemy_y[i]
            ...
            if smart_action == ACTION_BUILD_SUPPLY_DEPOT:
                ...
                if do_it and _ATTACK_MINIMAP in obs.observation.available_actions:
                    ...
                    if len(obs.observation.multi_select) > 0:
                        #("multi_observations: " + str(obs.observation.multi_select))
                        self.previous_observations = obs.observation.single_select[0][1]
            ...
            if smart_action == ACTION_BUILD_BARRACKS or smart_action == ACTION_BUILD_SUPPLY_DEPOT:
                ...
                if _MOVE_CAMERA in obs.observation.available_actions:
                    if self.enemyx > 0:
                        screnemy_y, screnemy_x = (obs.observation['feature_screen'][5] == _PLAYER_HOSTILE).nonzero()
                        print("screnemy_y" + str(screnemy_y) + " screnemy_x" + str(screnemy_x))
                        return actions.FunctionCall(_MOVE_CAMERA, [(self.enemyx, self.enemyy)])


        #return actions.FunctionCall(_MOVE_CAMERA, [self.enemyx[0], self.enemyy[0]])
        return actions.FunctionCall(_NO_OP, [])