...
class FullyConv:
    ...
    #def __init__(self, num_actions, use_lstm, input_data_format, use_data_format='NHWC'):
    ...
    def build(self, screen_input, minimap_input, nonspatial_input, available_mask_input,
        ...
        print("nonspatial out:")
        print(nonspatial_out)
    ...
    def _build_cnn_block(self, input, all_features, our_features):
        feature_layers = tf.split(input, int(input.shape[1]), axis=1)
        #feature_layers = tf.split(input, input.shape[1], -1)
    ...
    def _broadcast_features(self, nonspatial_features, size):
        # print("TYPE: ", type(size), "VALUE: ", size)
        # print(tf.expand_dims(nonspatial_features, 2))
        # print(tf.expand_dims(tf.expand_dims(nonspatial_features, 2), 3))
        return tf.tile(tf.expand_dims(tf.expand_dims(nonspatial_features, 2), 3), [1, 1, int(size), int(size)])
